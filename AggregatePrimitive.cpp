...
void AggregatePrimitive::setValue(std::vector<GeometricPrimitive> _list){
	primitiveList = _list;
}
...
bool AggregatePrimitive::intersect(Ray& ray, float* thit, Intersection* in){
	...
	for (std::vector<GeometricPrimitive>::iterator it = primitiveList.begin(); it != primitiveList.end(); it++){
