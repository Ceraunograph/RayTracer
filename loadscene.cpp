...
// create our object primitive array here
// create our light array here
// createCamera
//
/*
Camera camera;
...
void loadScene(std::string file) {
  ...
  if(!inpfile.is_open()) {
	...
	toCamera = rotate4 * rotate3 * rotate2 * rotate1 * translate;
	...
	Shape* shape;
	shape->setValue(atof(splitline[1].c_str()), atof(splitline[2].c_str()), atof(splitline[3].c_str()), atof(splitline[4].c_str()), NULL, NULL, NULL, 0.0);
	...
	TMatrix matrix;
	TMatrix inverse;
	matrixStack.pop(matrix, inverse);
	inverse.inverse();
	

	GeometricPrimitive sphere;
	sphere.setValue(inverse, matrix, shape, material);

	primitive.insert(sphere);
	...
	TMatrix matrix;
	TMatrix inverse;
	matrixStack.peek(matrix, inverse);
	inverse.inverse();
	...
	matrixStack = scale * matrixStack;
      ...
      //   the camera to preserve the “identity” transformation.
        ...
        //mst.push();
	matrixStack =  unitMatrix * toCamera;
      }
      //popTransform
	...
	Light light;
	light.setValue(NULL, dir, true, false, color);
	...
	color.setValue(atof(splitline[4].c_str()), atof(splitline[5].c_str()), atof(splitline[6].c_str()));
	Light light;
	light.setValue(pos, NULL, false, true, color);
	lights.push_back(light);
